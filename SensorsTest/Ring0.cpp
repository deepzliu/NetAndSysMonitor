...
bool Ring0::ExtractDriver(void)
{
   ...
   DWORD dwRetVal = GetTempPath(sizeof(temppath),temppath);
...
bool Ring0::Open(void)
{
	...
	CString drivername = "\\\\.\\WinRing0_1_2_0";
    ...
    if(driver==INVALID_HANDLE_VALUE)
	{
       ...
       if (InstallWinIoDriver(szWinIoDriverPath,true))
	   {
		  if(!StartWinIoDriver())
              return false;
	      driver = CreateFile(drivername,GENERIC_READ|GENERIC_WRITE,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
    ...
    isMutex = new CMutex(false,"Access_ISABUS.HTP.Method");
	return true;
}
