...
BOOL CSmartReader::ReadSMARTInfo(BYTE ucDriveIndex)
{
	...
	sprintf(szT1,"\\\\.\\PHYSICALDRIVE%d",ucDriveIndex);
...
BOOL CSmartReader::IsSmartEnabled(HANDLE hDevice,UCHAR ucDriveIndex)
{
	...
	{
		dwRet=GetLastError();
		m_stDrivesInfo[ucDriveIndex].m_csErrorString.Format(L"Error %d in reading SMART Enabled flag",dwRet);
	}
	return bRet;
}
...
VOID CSmartReader::ConvertString(PBYTE pString,DWORD cbData)
{
	...
	//lstrcpyA(szT1,(PCHAR)(LPCTSTR)csT1);
	lstrcpyA(szT1,csT1.GetBuffer()); //(PCHAR)(LPCTSTR)
...
//	if(IsDebuggerPresent()==FALSE)
	{
		GetModuleFileNameA(NULL,szINIFileName,MAX_PATH);
		szINIFileName[lstrlenA(szINIFileName)-3]=0;
		lstrcatA(szINIFileName,"ini");
	}
//	else
//		wsprintf(szINIFileName,"D:\\Saneesh\\Projects\\Helpers\\SMART\\Smart.ini");
	nSmartAttribs=GetPrivateProfileIntA("General","Max Attributes",0,szINIFileName);
	for(nC1=0;nC1<nSmartAttribs;++nC1)
	{
		sprintf(szKeyName,"Attrib%d",nC1);
		stSD.m_ucAttribId=GetPrivateProfileIntA(szKeyName,"Id",0,szINIFileName);
		stSD.m_bCritical=GetPrivateProfileIntA(szKeyName,"Critical",0,szINIFileName);
		GetPrivateProfileStringA(szKeyName,"Name","",szValue,1024,szINIFileName);
		stSD.m_csAttribName=szValue;
		if(nC1==16 && stSD.m_csAttribName == "") 
			stSD.m_csAttribName = "Temperature";
		GetPrivateProfileStringA(szKeyName,"Details","",szValue,1024,szINIFileName);
