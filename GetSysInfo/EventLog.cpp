...
#define PROVIDER_NAME           L"Security" //L"MyEventProvider"
#define RESOURCE_DLL            L"adtschema.dll" //L"<path>\\Provider.dll"
...
void GetLoginEventLog(void)
{
    ...
    while (ERROR_SUCCESS == status)
    {
        ...
        else
        {
            ...
            if(DumpRecordsInBuffer(pBuffer, dwBytesRead) == 1) goto cleanup; 
			//printf("%s\n", pBuffer);
        }
    }
...
DWORD DumpRecordsInBuffer(PBYTE pBuffer, DWORD dwBytesRead)
{
    ...
    while (pRecord < pEndOfRecords)
    {
		...
		if(1)//((PEVENTLOGRECORD)pRecord)->EventType == 0x10 || ((PEVENTLOGRECORD)pRecord)->EventType == 0x8)
        {
            ...
            if (pMessage)
            {
				if(((PEVENTLOGRECORD)pRecord)->EventCategory == 0x3002){
					status = ApplyParameterStringsToMessage(pMessage, pFinalMessage);
				}

				if(pFinalMessage){
					w2c(msgbuf, pFinalMessage, wcslen(pFinalMessage));
				}else{
					w2c(msgbuf, pMessage, wcslen(pMessage));
				}
		...
		if(--count == 0) return 1; //The count of logs to display
		Sleep(1000);
    }
